  ...
  describe 'handles the exception' do
    ...
    describe '::JWT::VerificationError' do
      ...
      it 'returns a 401 and the correct error msg' do
        ...
        expect(body).to eq(success: false, error: 'Invalid JWT token : Signature Verification Error')
      end
    end
    ...
    describe '::JWT::ExpiredSignature' do
      ...
      it 'returns a 401 and the correct error msg' do
        expect(last_response.status).to eq 401
        body = JSON.parse(last_response.body, symbolize_names: true)
        expect(body).to eq(success: false, error: 'Invalid JWT token : Expired Signature (exp)')
      end
    end
    ...
    describe '::JWT::IncorrectAlgorithm' do
      ...
      it 'returns a 401 and the correct error msg' do
        expect(last_response.status).to eq 401
        body = JSON.parse(last_response.body, symbolize_names: true)
        expect(body).to eq(success: false, error: 'Invalid JWT token : Incorrect Key Algorithm')
      end
    end
    ...
    describe '::JWT::ImmatureSignature' do
      ...
      it 'returns a 401 and the correct error msg' do
        expect(last_response.status).to eq 401
        body = JSON.parse(last_response.body, symbolize_names: true)
        expect(body).to eq(success: false, error: 'Invalid JWT token : Immature Signature (nbf)')
      end
    end
    ...
    describe '::JWT::InvalidIssuerError' do
      ...
      it 'returns a 401 and the correct error msg' do
        expect(last_response.status).to eq 401
        body = JSON.parse(last_response.body, symbolize_names: true)
        expect(body).to eq(success: false, error: 'Invalid JWT token : Invalid Issuer (iss)')
      end
    end
    ...
    describe '::JWT::InvalidAudError' do
      ...
      it 'returns a 401 and the correct error msg' do
        expect(last_response.status).to eq 401
        body = JSON.parse(last_response.body, symbolize_names: true)
        expect(body).to eq(success: false, error: 'Invalid JWT token : Invalid Audience (aud)')
      end
    end
    ...
    describe '::JWT::InvalidSubError' do
      ...
      it 'returns a 401 and the correct error msg' do
        expect(last_response.status).to eq 401
        body = JSON.parse(last_response.body, symbolize_names: true)
        expect(body).to eq(success: false, error: 'Invalid JWT token : Invalid Subject (sub)')
      end
    end
    ...
    describe '::JWT::InvalidJtiError' do
      ...
      it 'returns a 401 and the correct error msg' do
        expect(last_response.status).to eq 401
        body = JSON.parse(last_response.body, symbolize_names: true)
        expect(body).to eq(success: false, error: 'Invalid JWT token : Invalid JWT ID (jti)')
      end
    end
    ...
    describe '::JWT::DecodeError' do
      ...
      it 'returns a 401 and the correct error msg' do
        expect(last_response.status).to eq 401
        body = JSON.parse(last_response.body, symbolize_names: true)
        expect(body).to eq(success: false, error: 'Invalid JWT token : Decode Error')
      end
    end
  end
end
